                   ...
                   unsigned calledpage[], int is_replace)
{
    ...
    //determine if a function end exists.
...
void code_preprocess(Mem * mem, unsigned pageSize, range * c, int *dsmPage)
{
    ...
    unsigned pAddr = vtop(mem->mem, mem->mem_size, mem->pgd, start);
    ...
    printf("last_1_idx %d\n", last_1_idx);
#endif
    memcpy(dsmPage, is_dism, last_1_idx * sizeof(int));
}
...
//determine if the page is code page by opcode "push ebp; mov esp, ebp" instruction  //linux
            //windows: mov %edi,%edi
        ...
        if (offset > end) {
            ...
            if (disasn_replace
                (mem, page, &end, pageSize, 0, vaddr, NULL,
                 is_replace) == 0) {
#ifdef DEBUG
                printf("start disasn and replace %x %x %x\n", vaddr + offset,
...
void page_init(Mem * mem, int page_idx, int page_size, int * dsmPages,
               int is_print, unsigned vaddr, unsigned *calledpage)
{
    char *page = (char *) ((unsigned) mem->mem + page_idx * page_size);
    unsigned end;

    dsmPages[page_idx] = 0;

    end = page_end[page_idx];

    node *next;
    for (next = offsets[page_idx]; next != NULL; next = next->next) {
        unsigned offset = next->offset;
        if (offset > end) {

            end = offset;
            if (disasn_replace
                (mem, page, &end, page_size, is_print, vaddr, calledpage, 0) == 0) {
                //   dsmPages[pageIndex] = 1;
            } else {
                end = offset;
            }
        }
    ...
    if (end >= page_size) {
        page_end[page_idx + 1] = end - page_size;
    }
#ifdef DEBUG
    if (dsmPages[page_idx] == 0)
        printf("can't disassemble %d\n", page_idx);
#endif
}

//find the function begin and end in page.
void code_init(Mem * mem, unsigned vaddr, unsigned pageSize,
               int *dsmPages, unsigned * virtualAddrs, int is_print,
               unsigned *calledpage, unsigned *codePageNo)
{
    ...
    if (is_code_page(mem, vaddr, pageSize, virtualAddrs) == 0) {
        unsigned paddr = vtop(mem->mem, mem->mem_size, mem->pgd, vaddr);
        int pageIndex = paddr / pageSize;
        page_init(mem, pageIndex, pageSize, dsmPages , is_print, vaddr, calledpage);
        
        if(dsmPages[pageIndex] == 1){
          char *page = (char *) ((unsigned) mem->mem + paddr);
          page_init2(mem, page, pageIndex, pageSize, vaddr, 1);
        }
