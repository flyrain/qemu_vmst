...
const CPULogItem cpu_log_items[] = {
    ...
    { CPU_LOG_MMU, "memory_falut",
      "show CPU state when memory fault happens" },
    { CPU_LOG_CR3, "CR3",
      "show CR3 changes, with esp values" },
#endif
#ifdef DEBUG_IOPORT
...
//zlin
    ...
    for(i = 0; i < CPU_TLB_SIZE; i++) {
        ...
        for (mmu_idx = 0; mmu_idx < NB_MMU_MODES; mmu_idx++) {
            ...
            //zlin.begin
            env->vmmi_tlb_table[mmu_idx][i] = s_cputlb_empty_entry;
            //zlin.end
        }
    }
...
//zlin
	if(qemu_loglevel_mask(CPU_LOG_MMU))
	{
    	qemu_log("tlb_flush_page: " TARGET_FMT_lx "\n", addr);
    }	
...
//zlin
		if(qemu_loglevel_mask(CPU_LOG_MMU))
       		qemu_log("tlb_flush_page: forced full flush ("
               TARGET_FMT_lx "/" TARGET_FMT_lx ")\n",
               env->tlb_flush_addr, env->tlb_flush_mask);
    ...
    for (mmu_idx = 0; mmu_idx < NB_MMU_MODES; mmu_idx++){
		...
		tlb_flush_entry(&env->vmmi_tlb_table[mmu_idx][i],addr);
	}
        ...
        for (mmu_idx = 0; mmu_idx < NB_MMU_MODES; mmu_idx++) {
            for(i = 0; i < CPU_TLB_SIZE; i++)
                tlb_reset_dirty_range(&env->tlb_table[mmu_idx][i],
                                      start1, length);
                //yang
                tlb_reset_dirty_range(&env->vmmi_tlb_table[mmu_idx][i],
                                      start1, length);
        }
    }
}
        ...
        for(i = 0; i < CPU_TLB_SIZE; i++)
          {
            ...
            //yang
            tlb_update_dirty(&env->vmmi_tlb_table[mmu_idx][i]);
          }
    }
}
    ...
    for (mmu_idx = 0; mmu_idx < NB_MMU_MODES; mmu_idx++)
      {
        ...
        //yang
        tlb_set_dirty1(&env->vmmi_tlb_table[mmu_idx][i], vaddr);
      }
}
...
void vmmi_tlb_set_page_old(CPUState *env, target_ulong vaddr,
                  target_phys_addr_t paddr, int prot,
                  int mmu_idx, target_ulong size, uint32_t is_copy)
{
    unsigned int index;
	...
	if(is_copy)
#endif
    	addend = (unsigned long)((paddr + (unsigned long)vmmi_mem_shadow) & TARGET_PAGE_MASK);
#ifdef VMMI_COW
	else
    	addend = (unsigned long)((paddr + (unsigned long)vmmi_mem) & TARGET_PAGE_MASK);
#endif
	//yang.end
...
/* Add a new TLB entry. At most one entry for a given virtual address
   is permitted. Only a single TARGET_PAGE_SIZE region is mapped, the
   supplied size is only used by tlb_flush_page.  */
void vmmi_tlb_set_page(CPUState *env, target_ulong vaddr,
                  target_phys_addr_t paddr, int prot,
                  int mmu_idx, target_ulong size)
{
    PhysPageDesc *p;
                                ...
                                uint32_t val)
{
    ...
    //yufei.begin
    if(io_need_red)
      stb_p(vmmi_mem_shadow + ram_addr, val);
    else
                                ...
                                uint32_t val)
{
    ...
    //yufei
    if(io_need_red)
      stw_p(vmmi_mem_shadow + ram_addr, val);
    else
                                ...
                                uint32_t val)
{
    ...
    //yufei
    if(io_need_red){ 
      stl_p(vmmi_mem_shadow + ram_addr, val);
    }else
...
void cpu_physical_memory_rw2(target_phys_addr_t addr, uint8_t *buf,
                            int len, int is_write)
{
    int l, io_index;
    uint8_t *ptr;
                                         ...
                                         enum device_endian endian)
{
    ...
    if ((pd & ~TARGET_PAGE_MASK) > IO_MEM_ROM &&
		...
		//zlin.begin
		//if(is_monitored_vmmi_kernel_data_read())
		{
//			ptr=vtop(addr);
		}
//		else
