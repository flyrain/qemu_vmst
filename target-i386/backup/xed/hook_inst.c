...
/* XED_REG_CS */ {cs_reg, -1, -1},
/* XED_REG_DS */ {ds_reg, -1, -1},
/* XED_REG_ES */ {es_reg, -1, -1},
/* XED_REG_SS */ {ss_reg, -1, -1},
/* XED_REG_FS */ {fs_reg, -1, -1},
/* XED_REG_GS */ {gs_reg, -1, -1},
/* XED_REG_XMM0 */ {-1, -1, -1},
/* XED_REG_XMM1 */ {-1, -1, -1},
/* XED_REG_XMM2 */ {-1, -1, -1},
/* XED_REG_XMM3 */ {-1, -1, -1},
	...
	case XED_OPERAND_MEM1:{
			...
			if (seg_regid != XED_REG_INVALID) {
				...
				if (!default_segment) {
					...
					segbase =
					    TEMU_cpu_segs[segmentreg].base;
					segsel =
					    TEMU_cpu_segs[segmentreg].selector;
				}
//          fprintf(tracelog,"memlen %d SEG %d %s, default_seg %d segbase %x, segsel %d ",memlen, seg_regid, \
			...
			if (base_regid != XED_REG_INVALID) {
				int basereg =
				    xed2chris_regmapping[base_regid][1];
...
//          fprintf(tracelog,"BASE_REG %d %s, base %x ",base_regid, xed_reg_enum_t2str(base_regid), base);
			...
			if (mem_idx == 0 && index_regid != XED_REG_INVALID) {
				int indexreg =
				    xed2chris_regmapping[index_regid][1];
...
static void Instrument_MOV(INS xi) {
	...
	fprintf(tracelog, "D 0x%08x 0x%08x\n", savedeip, global_time_stamp);
	...
	if (operand_is_reg(op_name, &reg_id)) {
		...
		if (taint) {
			fprintf(tracelog,
				"U 0x%08x 0x%08x %02d - 0x%08x 0x%08x\n",
				record[0].value_tag, record[0].time_stamp,
				record[0].taint_size, savedeip, global_time_stamp);
		}
		set_reg_taint(reg_id);

		#ifdef DEBUG_TAINT
		int regnum = xed2chris_regmapping[reg_id][1];
		value = TEMU_cpu_regs[regnum];
		fprintf(tracelog, "%d R %d %x\n", 1, reg_id, value);
		taint = get_reg_taint(reg_id, &record);
		if (taint) {
			fprintf(tracelog,
				"GetRegTaint 0x%08x 0x%08x %d with 0x%08x has taint = %d\n",
				record[0].value_tag, record[0].time_stamp,
		...
		else if (operand_is_mem(op_name0, &mem_addr, 0)) {
			set_virtmem_taint(mem_addr, width);
		}
	}

	else if (operand_is_mem(op_name, &mem_addr, 1)) {
		taint_record_t record[4];
		uint64_t taint = get_virtmem_taint(mem_addr, 1, &record);
		if (taint) {
			fprintf(tracelog,
				"U 0x%08x 0x%08x %02d - 0x%08x 0x%08x \n",
				record[0].value_tag, record[0].time_stamp,
			...
			set_virtmem_taint(mem_addr, 4);
			#ifdef DEBUG_TAINT
			fprintf(tracelog, "%d M %x, taint %d\n", 1, mem_addr, \
				taint);
			#endif
		}
	} else {
...
void UnimplementedInstruction(INS ins) {
...
static void Instrument_ADD(INS ins) {
}
...
static void Instrument_CALL_NEAR(INS ins) {
	...
	curr_basic_block_end_type = T_CALL;
...
//	set_addr_type(esp-4, T_RET_ADDR);

	if (operand_is_reg(op_name, &reg_id)) {
		int regnum = xed2chris_regmapping[reg_id][1];
		target_eip = TEMU_cpu_regs[regnum];
	} else if (operand_is_relbr(op_name, &branch)) {
		target_eip =
		    branch + xed_decoded_inst_get_length(&xedd_g) +
		    *TEMU_cpu_eip;

	} else if (operand_is_mem(op_name, &mem_addr, 0)){
		read_mem(mem_addr, 4, &target_eip);
		#if 0
		taint_record_t record[4];
		uint64_t taint = get_virtmem_taint(mem_addr, 1, &record);
		if (taint) {
			fprintf(tracelog, "T 0x%08x 0x%08x 0x%08x \n",
				record[0].value_tag, record[0].time_stamp,
				T_FUN_PTR);
			fprintf(tracelog,
				"M 0x%08x %02d 0x%08x 0x%08x %02d r\n", mem_addr, 4,
				record[0].value_tag, record[0].time_stamp,
				record[0].taint_size);
		} else
		{
			fprintf(tracelog, "D 0x%08x 0x%08x \n", savedeip, global_time_stamp);
			fprintf(tracelog, "T 0x%08x 0x%08x 0x%08x \n", savedeip, global_time_stamp, T_FUN_PTR);
			set_virtmem_taint(mem_addr, 4);
			fprintf(tracelog,
				"M 0x%08x %02d 0x%08x 0x%08x %02d r\n", mem_addr, 4,
				savedeip, global_time_stamp, 4);
			#ifdef DEBUG_TAINT
			fprintf(tracelog, "Call near unknown\n");
			#endif
		}
		#endif
	}
	...
	struct names_t *names = query_name(target_eip);
	if ((names != NULL))
		fprintf(tracelog, "C 0x%08x 0x%08x %s::%s\n", esp,
			target_eip, names->mod_name, names->fun_name);
	else
		fprintf(tracelog, "C 0x%08x 0x%08x U\n", esp, target_eip);


	//insert return address
	insert_addr_value (esp, xed_decoded_inst_get_length(&xedd_g) + *TEMU_cpu_eip);

	if(target_eip > LIBRARY_BOUNDARY)
	{
		is_library_call = 1;
	}

	//push_call_stack(p_call_stack_g, target_eip, esp);

//	fprintf(tracelog,"PUSH_STACK to call stack %p\n",p_call_stack_g);
...
static void Instrument_Jcc(INS ins) {
	...
	curr_basic_block_end_type = T_JMP;
	is_cond_jmp_inst = 1;

	xed_reg_enum_t reg_id;
	uint32_t value;
	uint32_t mem_addr;
//	uint32_t inst_length = xed_decoded_inst_get_length(&xedd_g);
//	next_ins_addr = savedeip + inst_length;
	uint32_t jmp_target;

	const xed_operand_t *op = xed_inst_operand(ins, 0);
	xed_operand_enum_t op_name = xed_operand_name(op);

	if(operand_is_relbr(op_name, &jmp_target_ins_addr))
	{
		jmp_target_ins_addr = jmp_target_ins_addr + next_ins_addr;
		fprintf(tracelog, "next eip %x jmp target %x\n",next_ins_addr, jmp_target_ins_addr);
	}
...
static void Instrument_LEA(INS xi) {
	...
	if (operand_is_reg(op_name, &reg_id)) {
		...
		if (operand_is_mem(op_name, &mem_addr, 1)) 
		{
			fprintf(tracelog, "D 0x%08x 0x%08x\n", savedeip, global_time_stamp);
			set_reg_taint(reg_id);
			fprintf(tracelog,
...
static void Instrument_PUSH(INS xi) {
...
static void Instrument_RET_NEAR(INS ins) {
	is_ret_inst = 1;
}
