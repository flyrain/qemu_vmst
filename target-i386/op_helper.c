...
void helper_iret_protected(int shift, int next_eip)
{
	...
	//yang
//	if(is_ins_log()&&current_pc==0xc100321e)
//		printf("hello\n");
...
#define MMUSUFFIX _kmmu
#define MMUSUFFIX_kmmu
...
//zlin.begin
void vmmi_tlb_fill(target_ulong addr, int is_write, int mmu_idx, void *retaddr)
{
    TranslationBlock *tb;
    int ret;
    unsigned long pc;
...
char cr3_name[256];
uint32_t take_snapshot=0;
uint32_t snapshot_cr3;
uint32_t take_snapshot_esp;
...
int save_snapshot()
{
    ...
    uint32_t l;
    uint8_t buf[1024];
    uint32_t size = snapshot_size;
    const char *filename = snapshot_name;
    snapshot_size = size;
    ...
    while (size != 0) {
        ...
        if (fwrite(buf, 1, l, f) != l) {
            printf("fwrite() error in do_physical_memory_save\n");
            goto exit;
        }
        fflush(f);
        addr += l;
...
extern void my_monitor_disas(target_ulong pc);
#include "hook_inst.h"
...
int vmac_memory_read(target_ulong addr, uint8_t *buf, int len)
{
	...
	while(len>0){
		page = vmmi_vtop(addr);
		if(page==0xffffffff) return -1;
		phys_addr = (target_phys_addr_t)vmmi_mem_shadow + page;	

		l = (addr &TARGET_PAGE_MASK)+TARGET_PAGE_SIZE -addr;
		if(l>len)
			l = len;
		memcpy(buf, phys_addr, l);
...
int vmac_memory_write(target_ulong addr, uint8_t *buf, int len)
{
