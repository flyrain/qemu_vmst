...
char tempbuf[120];
    ...
    char *current_task, **argvp;
    int pid, len, i=0, policy =0; 

			current_task = myesp & (~8191UL);
#ifdef FC5
//			fprintf(logfile, "sys_enter current_thread 0x%08x (tb->pc 0x%08x); ", current_task, tb->pc);
			paddr = cpu_get_phys_page_debug(env, current_task);
			cpu_physical_memory_read(paddr, &current_task, 4);
	//		fprintf(logfile, "current_task 0x%08x \n", current_task);
			if ( current_task < 0xc0000000 ) goto syscall_end;

			// if (current_task < 0xc0000000)  goto syscall_end;
#endif
			pid = 0;
			paddr = cpu_get_phys_page_debug(env, current_task+TS_PID);
			cpu_physical_memory_read(paddr, &pid, 4);

			//if (pid <=0)  goto syscall_end;

			bzero(command, 120);
			paddr = cpu_get_phys_page_debug(env, current_task+TS_COMM);
			if (paddr!=-1) {
				cpu_physical_memory_read(paddr, command, 16);
				 // fprintf(logfile,"PID %3d (%16s)", pid, command);
				if ( strcmp(command,"evills") == 0) {
					policy = 1;
				}
			}
/*
		    if ((pid == 1) && (start_logging==0))
			start_logging =1;
*/
			...
			//current_task = ESP & (~8191UL);
                    paddr = cpu_get_phys_page_debug(env, current_task+TS_SIZE);
                    cpu_physical_memory_write(paddr, &(env->eip_check), 4); 
			current_pid=pid;
//		    fprintf(logfile, "current_task 0x%08x\n", current_task);
	//	    fprintf(logfile, "==================PID %d, syscall %d, eip_check 0x%08x (paddr 0x%08x), current_task 0x%08x\n", pid, EAX, env->exception_next_eip, paddr, current_task);

	   	    //TS_SIZE = sizeof(struct task_struct) + 0 --> exception_next_eip
	   	    //TS_SIZE+4 = sizeof(struct task_struct) + 4 --> pid
	   	    //TS_SIZE+8 = sizeof(struct task_struct) + 8 --> syscall number
	   	    //Dump format: PID %3d (%16s)[%14s] \n
/*
                if (tb->pc == (target_ulong)(SYSENTER_PC))
                        fprintf(logfile, "s");
		else
                        fprintf(logfile, "t");
*/
...
#define EAX_XUXIAN (env->regs[R_EAX])
		   // if (start_logging)
	     	    switch (EAX) {	
				...
				//env->eip_check = env->exception_next_eip;
                    		paddr = cpu_get_phys_page_debug(env, current_task+TS_SIZE);
                        	cpu_physical_memory_write(paddr, &(env->exception_next_eip), 4); 
                    		paddr = cpu_get_phys_page_debug(env, current_task+TS_SIZE+4);
                        	cpu_physical_memory_write(paddr, &pid, 4); 
				pid = EAX;
                    		paddr = cpu_get_phys_page_debug(env, current_task+TS_SIZE+8);
                        	cpu_physical_memory_write(paddr, &pid, 4); 
				break;
                    	case 3 : // sys_readG
				fprintf(logfile,"PID %3d (%16s)", pid, command);
				...
				pid = EAX;
                        	...
                        	cpu_physical_memory_write(paddr, &pid, 4); 
				pid = ECX;
                    		paddr = cpu_get_phys_page_debug(env, current_task+TS_SIZE+12);
                        	cpu_physical_memory_write(paddr, &pid, 4); 
				break;
                    	case 4 : // sys_write
				{
				...
				if (policy) {
				    fprintf(logfile,"PID %3d (%16s)", pid, command);
				    fprintf(logfile," [POLICY: ");
				    if (strncmp(tempbuf, "/secrets", 8) == 0 ){
				        fprintf(logfile,"DENIED   ]");
					decision = 1; //DENIED
				    }else
				        fprintf(logfile,"PERMITTED]");
				    fprintf(logfile," %s\n", tempbuf);
				}
...
/*
				    ...
				    if ( paddr!=-1) {
                        	        cpu_physical_memory_write(paddr, &(env->exception_next_eip), 4); 
					//fprintf(logfile,"Saving eip_check 0x%x\n", env->exception_next_eip);
                            		if (paddr!=-1) {
					 	paddr = cpu_get_phys_page_debug(env, ESP & (~8191UL) + TS_SIZE);
                                        	cpu_physical_memory_read(paddr, &(env->eip_check), 4);
                                        	//fprintf(logfile,"Saving eip_check 0x%x:0x%x (PID %d)\n", env->eip_check, env->exception_next_eip,  pid);
                            		}
				    }
*/
		    		} 
...
*/
				...
				case 5: // accept
					...
					fprintf(logfile,"\n");
				        ...
				        cpu_physical_memory_write(paddr, &pid, 4); 
				        pid = *(int*) (tempbuf+4);
				        paddr = cpu_get_phys_page_debug(env, current_task+TS_SIZE+16);
				        cpu_physical_memory_write(paddr, &pid, 4); 
					break;
				case 6: // getsockname
				...
				case 10: // recv
					...
					if (paddr!=-1)	{
					   ...
					   cpu_physical_memory_write(paddr, &pid, 4); 
					   paddr = cpu_get_phys_page_debug(env, current_task+TS_SIZE+16);
					   cpu_physical_memory_write(paddr, &msg, 4); 
				...
				fprintf(logfile,"%s\n", tempbuf);
                        ...
                        case 142 : // sys_select
				{ int i; fd_set fds; 
                                fprintf(logfile,"PID %3d (%16s)[sys_select  142]: nfd %d; ", 
					pid, command, EBX);
				if (ECX) {
					paddr = cpu_get_phys_page_debug(env, ECX);
					cpu_physical_memory_read(paddr, &fds, sizeof(fd_set)); 
				...
				if (ESI) {
					paddr = cpu_get_phys_page_debug(env, ESI);
					cpu_physical_memory_read(paddr, &fds, sizeof(fd_set)); 

					fprintf(logfile, "errorfds ["); 
					for (i=0; i<EBX; i++) {
				    	    if ( FD_ISSET(i, &fds) )
				    		fprintf(logfile," %d", i);
					}
				...
				fprintf(logfile,"\n");
                        ...
                        case 145 : // sys_readv
                                ...
                                break;
                        case 146 : // sys_writev
				{ int i, len, nfds; struct iovec *iovp; 
				...
				if (EDX>0) {
				    ...
				    bzero(tempbuf, 120);
				    nfds = EDX;
                                    if (EDX*sizeof(struct iovec) >= 120)
                                                nfds = 120/sizeof(struct iovec) -1;
				    cpu_physical_memory_read(paddr, tempbuf, nfds*sizeof(struct iovec)); //-> get the iov list
				    iovp = (struct iov*) tempbuf;
				    for(i=0; i<nfds; i++, iovp++) {
				...
				fprintf(logfile,"PID %3d (%16s)[sys_tkill   238]: pid %d; sig %d\n", pid, 
					command, EBX, ECX);
                        ...
                        case 259 : // sys_timer_create
                                ...
                                break;
			default:
                                fprintf(logfile,"PID %3d (%16s)[sys_unknown %3d]\n", pid, command, EAX);
				break;
                    	}  //switch
