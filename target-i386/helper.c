...
//zlin
	if(qemu_loglevel_mask(CPU_LOG_MMU))
    	qemu_log("CR0 update: CR0=0x%08x\n", new_cr0);
#if defined(DEBUG_MMU)
...
struct cr3_node *cr3_hash = NULL;

static void add_cr3_node(target_ulong cr3, target_ulong esp)
{
	struct cr3_node *n;

	n = malloc(sizeof(struct cr3_node));
	n->cr3 = cr3;
	n->esp = esp;
	HASH_ADD_INT (cr3_hash, cr3, n);
}

static struct cr3_node * find_cr3(target_ulong cr3)
{
	struct cr3_node *n;
	HASH_FIND_INT(cr3_hash,  &cr3, n);
	return n;
}

uint32_t vmmi_start = 0;
uint32_t vmmi_main_start=0;
target_ulong vmmi_process_cr3 = 0xffffffff;
...
//yufei.begin
extern uint32_t io_need_red; 

inline void set_io_need_red(){
  io_need_red =1;
}
//yufei.end
...
//zlin
	if(qemu_loglevel_mask(CPU_LOG_MMU))
	    qemu_log("MMU fault: addr=" TARGET_FMT_lx " w=%d u=%d eip=" TARGET_FMT_lx "\n",
...
/* return value:
   ...
   1  = generate PF fault
*/
int vmmi_cpu_x86_handle_mmu_fault(CPUX86State *env, target_ulong addr,
                             int is_write1, int mmu_idx)
{
    uint64_t ptep, pte;
    target_ulong pde_addr, pte_addr;
...
//	qemu_log("vmmi handle mmu fault");
//	qemu_log_flush();
    ...
    is_write = is_write1 & 1;
        ...
        pde = *(uint32_t*)((uint64_t)vmmi_mem_shadow + (uint64_t) pde_addr);
        ...
        if ((pde & PG_PSE_MASK) && (env->cr[4] & CR4_PSE_MASK)) {
...
#ifdef VMMI_COW
			is_copy = is_shadow_page_exist(pde); 
#endif
            page_size = 4096 * 1024;
			...
			pte = *(uint32_t*)((uint64_t)vmmi_mem_shadow + (uint64_t) pte_addr);
			...
			//yang	
#ifdef VMMI_COW
			is_copy = is_shadow_page_exist(pte); 
#endif
...
{
	uint32_t pgd = vmmi_cr3;
    uint32_t pde_addr = (pgd & ~0xfff) + ((addr >> 20) & ~3);
    ...
    {
        pte_addr = ((pde & ~0xfff) + ((addr >> 10) & 0xffc));
        if (pte_addr > snapshot_size)
        {
            return -1;
        }
        pte = *(uint32_t*)((uint64_t)vmmi_mem_shadow + pte_addr);
        if (!(pte & PG_PRESENT_MASK))
        {
            return -1;
        }
    ...
    uint32_t paddr  = (pte & TARGET_PAGE_MASK) + page_offset;
	if(paddr > snapshot_size){
		qemu_log("too large");
		return -1;
	}
...
uint32_t vmmi_vtop2(uint32_t addr, uint32_t size)
{
	uint32_t pgd = vmmi_cr3;
    uint32_t pde_addr = (pgd & ~0xfff) + ((addr >> 20) & ~3);
	...
	if(is_shadow_page_exist(pte)){
          qemu_log("copy");
          a = (uint64_t)vmmi_mem_shadow + paddr;
          uint64_t b;
          b = (uint64_t)vmmi_mem_shadow+paddr;
          qemu_log("copy vtop is (%x,%x)", *(uint32_t *)a, *(uint32_t *)b);
	}
	else
	{
          a = (uint64_t)vmmi_mem_shadow+paddr;
          qemu_log("vtop is (%x)", *(uint32_t *)a);
	}
